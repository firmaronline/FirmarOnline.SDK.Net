...
namespace FirmarOnline.Model.eSign
{
    ...
    /// Permite convertir a <see cref="Signature"/> o <see cref="WebSignature"/>
    ...
    {
        /// <summary>
        /// Convierte el JSON a <see cref="Signature"/> o <see cref="WebSignature"/>
        ...
        public override Signature Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
        {
            ...
            bool hasBiometricInfo = root.EnumerateObject().Any(p => string.Equals(p.Name, nameof(WebSignature.B64PublicBioData), caseOptions))
                                    && root.EnumerateObject().Any(p => string.Equals(p.Name, nameof(WebSignature.B64PrivateBioData), caseOptions));
            ...
            return hasBiometricInfo
                ? JsonSerializer.Deserialize<WebSignature>(json, optionsWithoutConverter)
        ...
        {
            if (value is WebSignature webSignature)
            {
...
public class SignatureJsonConverter : JsonConverter
{
    ...
    /// <summary>
    /// Lee el JSON devolviendo un <see cref="WebSignature"/> o un <see cref="Signature"/> en 
    ...
    /// <returns>Un <see cref="WebSignature"/> o un <see cref="Signature"/> en función de si
    /// el JSON contiene datos biométricos</returns>
    public override object ReadJson(JsonReader reader, Type objectType, object existingValue, JsonSerializer serializer)
    {
        JObject jObject = JObject.Load(reader);

        bool hasBiometricInfo = jObject.ContainsKey(nameof(WebSignature.B64PublicBioData)) &&
                                jObject.ContainsKey(nameof(WebSignature.B64PrivateBioData));
        ...
        return hasBiometricInfo
            ? jObject.ToObject<WebSignature>(noConverterSerializer)
