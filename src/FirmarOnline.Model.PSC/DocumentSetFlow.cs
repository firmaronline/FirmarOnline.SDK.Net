...
namespace FirmarOnline.Model.PSC
{
    ...
    /// </summary>
    [CustomValidation(typeof(DocumentSetFlow), nameof(ValidateDocumentSetFlow), ErrorMessage = "The documentSetFlow definition is not valid.")]
    public class DocumentSetFlow : DocumentSetFlowUrl
    {
        /// <summary>
        ...
        public static ValidationResult ValidateDocumentSetFlow(DocumentSetFlow documentSetFlow)
        {
                 ...
                 (documentSetFlow.AuthenticationType.HasValue && documentSetFlow.AuthenticationType.Value.UseSMS() ? 1 : 0)) > 1)
            {
                return new ValidationResult("Only the sending of an SMS or WhatsApp by envelope generated to authenticate the recipient(s) is allowed.",
                        new string[] { nameof(SendMethod), nameof(ActionType), nameof(AuthenticationType) });
            }
